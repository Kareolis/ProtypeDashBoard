...
def generate_changes(unit, current_time):
    ...
    if unit['health'] == 0:
        ...
        if random.random() < 0.1:  # 10% вероятность повреждения
            ...
            if unit['type'] == "Пехота":
                print(unit['type'] + "1")
                unit['damage'] = random.choice(["Ранение ноги", "Ранение головы", "Нет"])
            else:
                unit['damage'] = random.choice(["Пробоина", "Прямое попадание", "Нет"])
                print(unit['type'] + "0")
...
current_units = data.to_dict('records')  # Преобразуем DataFrame в список словарей
...
for _ in range(NUM_CHANGES):
    ...
    for unit in current_units:
        ...
        output_data.append(updated_unit.copy())  # Добавляем копию в выходные данные

    current_units = updated_units  # Обновляем текущее состояние объектов
